...
An installed JuliaBUGS has no checkout and reads a snapshot of this directory instead, declared as an artifact in `JuliaBUGS/Artifacts.toml`.
`snapshot.jl` builds that snapshot and updates the declaration. Run it when installed users should see a change here.
