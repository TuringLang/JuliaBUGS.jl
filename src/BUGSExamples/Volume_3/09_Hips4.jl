...
model_def = @bugs begin
    ...
    for i in 1:M  # loop over studies
        ...
        pC[i] = cexpexp(base[i] - logHR[i] / 2)
        pS[i] = cexpexp(base[i] + logHR[i] / 2)
    ...
    sigmah ~ truncated(dnorm(0.2, 400), 0, nothing)  # between-trial sd = 0.05 (prior constrained to be positive)
    ...
    for k in 1:K  # loop over strata
        for n in 1:2  # loop over protheses
            ...
            var"pi"[n, k, 1, 1] = 1 - var"lambda.op"
            var"pi"[n, k, 1, 2] = 0
            var"pi"[n, k, 1, 3] = 0
            var"pi"[n, k, 1, 4] = 0
            var"pi"[n, k, 1, 5] = var"lambda.op"

            # Marginal probability of being in each state at time t>1
            for t in 2:N
                for s in 1:S
                    var"pi"[n, k, t, s] = inprod(var"pi"[n, k, t - 1, :], Lambda[n, k, t, :, s])
                end
            end
        end
    end
