name = "Fire: A lognormal-Pareto distribution"

model_def = @bugs begin
    for i in 1:N
        dummy[i] = 0
        dummy[i] ~ dloglik(logLike[i])
        logLike[i] = log(r / phi(alpha * sigma)) * (1 - stepxtheta[i]) +
                     log(1 - r) * stepxtheta[i] +
                     (-0.5 * log(2 * pi) - log(x[i]) - log(sigma) -
                      0.5 * pow((log(x[i]) - mu) / sigma, 2)) *
                     (1 - stepxtheta[i]) +
                     (log(alpha) + alpha * log(theta) - (alpha + 1) * log(x[i])) *
                     stepxtheta[i]
    ...
    r = (sqrt(2 * pi) * alpha * sigma * phi(alpha * sigma)) /
        (sqrt(2 * pi) * alpha * sigma * phi(alpha * sigma) +
...
reference_results = (
    alpha = (mean = 1.329, std = 0.03223),
    sigma = (mean = 0.1973, std = 0.01248),
    theta = (mean = 1.21, std = 0.03241),
    xf = (mean = 4.971, std = 145.2)
)

fire = Example(name, model_def, original, data, inits, inits_alternative, reference_results)
