...
function __determine_array_sizes(
    var"##data#227"::NamedTuple{var"##KEYS#236",var"##VALUE_TYPES#237"}
) where {var"##KEYS#236",var"##VALUE_TYPES#237"}
    map((:N, :T)) do x
        if x ∉ var"##KEYS#236"
            error(
    ...
    var"##array_sizes#240" = let _MVec = JuliaBUGS.StaticArrays.MVector
        NamedTuple{var"##array_var_names#239"}((
            _MVec{2}([1, 1]),
            _MVec{1}([1]),
            _MVec{1}([1]),
            _MVec{2}([1, 1]),
    ...
    for v in intersect(var"##array_var_names#239", var"##KEYS#236")
    ...
    for i in 1:N
        for j in 1:T
            ...
            JuliaBUGS.determine_array_sizes_logical!(
                data, var"##array_sizes#240", :dN, i, j
            )
        end
    end
    for j in 1:T
        for i in 1:N
            ...
            JuliaBUGS.determine_array_sizes_logical!(
                data, var"##array_sizes#240", :Idt, i, j
            )
        end
        ...
        JuliaBUGS.determine_array_sizes_logical!(
            data, var"##array_sizes#240", Symbol("S.treat"), j
        )
        JuliaBUGS.determine_array_sizes_logical!(
            data, var"##array_sizes#240", Symbol("S.placebo"), j
        )
    end
    ...
    for j in 1:T
        JuliaBUGS.determine_array_sizes_logical!(
            data, var"##array_sizes#240", Symbol("dL0.star"), j
        )
    end
    return (
        (
        ...
        NamedTuple{keys(var"##array_sizes#240")}(Tuple.(values(var"##array_sizes#240"))),
    )
end

function __compute_transformed!(
    var"##evaluate_env#236"::NamedTuple{var"##ALL_VARS#237"}
) where {var"##ALL_VARS#237"}
    (;
        N,
        T,
        Y,
    ...
    while var"##added_new_val#238"
        ...
        for i in 1:N
            for j in 1:T
                ...
                JuliaBUGS.@try_compute dN[i, j] =
                    Y[i, j] * _step((t[j + 1] - var"obs.t"[i]) - eps) * fail[i]
            end
        end
        for j in 1:T
            for i in 1:N
            ...
            JuliaBUGS.@try_compute var"S.treat"[j] = pow(
                exp(-(sum(dL0[1:j]))), exp(beta * -0.5)
            )
            JuliaBUGS.@try_compute var"S.placebo"[j] = pow(
                exp(-(sum(dL0[1:j]))), exp(beta * 0.5)
            )
        end
