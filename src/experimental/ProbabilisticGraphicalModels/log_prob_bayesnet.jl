...
function create_log_posterior(net::BayesNet{T}, observations::Dict{Symbol,Float64}) where T
    ...
    function marginalize_recursive(nodes_to_marginalize::Vector{Symbol}, 
                                 current_values::Dict{Symbol,Float64},
                                 depth::Int = 0)  # Add depth parameter for indentation
        ...
        if isempty(nodes_to_marginalize)
            ...
            if haskey(current_values, :X1)
                ...
                println("$(indent)Computing prior P(X1=", current_values[:X1], ") = ", pdf(net.nodes[:X1], current_values[:X1]))
            end
            ...
            for (obs_node, obs_val) in observations
                ...
                if !isempty(parents) && all(p -> haskey(current_values, p), parents)
                    ...
                    println("$(indent)Computing P(", obs_node, "=", obs_val, "|parents) = ", 
            ...
            parent_values = [current_values[p] for p in parents]
            node_dist = isempty(parents) ? net.nodes[current_node] : 
                                         net.nodes[current_node](parent_values...)
            
            println("$(indent)Node distribution for ", current_node, " with parents ", parents)
            ...
            for val in support(node_dist)
                ...
                println("$(indent)Contribution from ", current_node, "=", val, 
                       ": ", p_val, " * ", child_prob, " = ", contribution)
            end
        ...
        if isempty(nodes_to_marginalize)
            ...
            for (node, obs_val) in observations
                ...
                if !isempty(parents)
                    ...
                    println("Added log probability for ", node, ": ", 
                           logpdf(node_dist, obs_val))
                end
            end
            return log_p
        else
...
evaluate_model(model_5_nodes, Dict(:X5 => 2.0), X1_values,
