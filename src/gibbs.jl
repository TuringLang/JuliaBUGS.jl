...
t, s = AbstractMCMC.step(rng, AbstractMCMC.LogDensityModel(l), HMC(0.1, 10); n_adapts=1, initial_params = [1.0])
ss = AbstractMCMC.sample(AbstractMCMC.LogDensityModel(l), HMC(0.1, 10), 1; chain_type = Nothing, n_adapts=1, initial_params = [1.0])

# ! deepcopy varinfo is expensive right now

# logical var values are still evaled, so don't really matter
...
function AbstractMCMC.step(
    ...
    for (var_group, sampler) in sampler.sampler_map
        p = LogDensityProblemsAD.ADgradient(ad_backend, MarkovBlanketBUGSModel(model, var_group))
