...
function _render_example_page(ex::BUGSExamples.BUGSExample)
    ...
    println(io, "ex = JuliaBUGS.BUGSExamples.", basename(ex.source_dir))
    println(io, "model_def = include(JuliaBUGS.BUGSExamples.path(ex, \"model.jl\"))")
    println(io, "model = compile(model_def, ex.data, ex.inits)")
    println(io, "```")
    println(io)
...
"""
    build_example_pages(output_dir) -> Pair{String,Any}

Generate per-example markdown files under `output_dir/<slug>.md` and return a
`"Examples" => [...]` entry suitable for `makedocs(pages=...)`. Sub-entries are
grouped by volume so the header dropdown shows "Volume 1", "Volume 2", … each
expanding to its examples.
"""
function build_example_pages(output_dir::String)
    ...
    for vol in sort!(collect(keys(by_volume)))
        push!(sections, "Volume $vol" => by_volume[vol])
    end
