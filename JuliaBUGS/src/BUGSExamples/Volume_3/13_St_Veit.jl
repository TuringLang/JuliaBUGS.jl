...
model_def = @bugs begin
    theta[1] ~ dunif(theta[2], var"theta.max")
    theta[2] ~ dunif(theta[3], theta[1])
    theta[3] ~ dunif(theta[9], theta[2])
    theta[4] ~ dunif(theta[9], var"theta.max")
    theta[5] ~ dunif(theta[7], var"theta.max")
    theta[6] ~ dunif(theta[7], var"theta.max")
    theta[7] ~ dunif(theta[9], theta7max)
    theta7max = min(theta[5], theta[6])
    theta[8] ~ dunif(theta[9], var"theta.max")
    ...
    for j in 1:5
        theta[j + 11] ~ dunif(0, var"theta.max")
    ...
    for i in 1:nDate
        ...
        var"theta.smooth"[i] = 10 * round(theta[i] / 10)
    end
end
