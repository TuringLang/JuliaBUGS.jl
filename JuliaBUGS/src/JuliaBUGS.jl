...
"""
    gen_chains(chain_type, model, samples, stats; kwargs...)

Lay the samples and statistics generated by `AbstractMCMC.sample` out in the requested
`chain_type`. `samples[i]` is the flat parameter vector of draw `i` and `stats[i]` is that
draw's sampler statistics as a `NamedTuple`; draws need not all report the same statistics,
and an empty collection means no draw carries any.
...
`gen_chains(model, samples, stats; kwargs...)` without a `chain_type` returns an
`MCMCChains.Chains`.
"""
