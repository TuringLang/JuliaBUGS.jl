...
`chain_type`. `samples[i]` is the flat parameter vector of draw `i` and `draw_stats[i]` is
that draw's sampler statistics as a `NamedTuple`; draws need not all report the same
statistics, and an empty collection means no draw carries any.
...
and `stats` is a `NamedTuple` of the statistics the sampler reported for that draw.
