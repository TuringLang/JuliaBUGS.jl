...
With the `MCMCChains` extension loaded,
`gen_chains(model, samples, stats_names, stats_values; kwargs...)` returns an
`MCMCChains.Chains`. FlexiChains output is handled by its JuliaBUGS package extension.
"""
