...
"""
    gen_chains(chain_type, model, samples, stats_names, stats_values; kwargs...)

Lay the samples and statistics generated by `AbstractMCMC.sample` out in the requested
`chain_type`.

With the `MCMCChains` extension loaded, `chain_type = MCMCChains.Chains` returns an
`MCMCChains.Chains`, where array-valued variables and statistics are flattened into scalar
columns. With the `FlexiChains` extension loaded, `chain_type = FlexiChains.VNChain` returns a
`FlexiChains.FlexiChain{VarName}` keyed by variable name, where array-valued variables are
stored whole. `chain_type = Vector{AbstractMCMC.ParamsWithStats}` returns one
`AbstractMCMC.ParamsWithStats` per draw and needs no extension.

`gen_chains(model, samples, stats_names, stats_values; kwargs...)` without a `chain_type`
returns an `MCMCChains.Chains`.
"""
function gen_chains end

"""
    transition_params_and_stats(model::BUGSModel, ts, sampler)

Unpack the transitions `ts` produced by `sampler` into the
`(param_samples, stats_names, stats_values)` triple that [`gen_chains`](@ref) consumes.

`param_samples[i]` holds the flat parameter vector of draw `i`, ordered as
`LogDensityProblems.logdensity` expects it. `stats_names` lists the sampler statistics
recorded for every draw and `stats_values[i]` holds their values for draw `i`. Statistic
values may be arrays; output formats that only store scalars are responsible for flattening
them.
...
Shared body of the `AbstractMCMC.bundle_samples` methods: unpack the transitions `ts` with
[`transition_params_and_stats`](@ref) and lay them out with [`gen_chains`](@ref).
"""
function bundle_transitions(chain_type, logdensitymodel, ts, sampler; kwargs...)
    model = Model.base_bugs_model(logdensitymodel)
    param_samples, stats_names, stats_values = transition_params_and_stats(
        model, ts, sampler
    )
    return gen_chains(
        chain_type, model, param_samples, stats_names, stats_values; kwargs...
    )
end
