...
"""
    gen_chains(chain_type, model, samples, draw_stats; kwargs...)

Lay the samples and statistics generated by `AbstractMCMC.sample` out in the requested
`chain_type`. `samples[i]` is draw `i` as [`transition_params_and_stats`](@ref) reported it
— the flat parameter vector, or the evaluation environment for the environment-based
samplers — and `draw_stats[i]` is that draw's sampler statistics as a `NamedTuple`; draws
need not all report the same statistics, and an empty collection means no draw carries any.
...
`gen_chains(model, samples, draw_stats; kwargs...)` without a `chain_type` returns an
`MCMCChains.Chains`.
"""
function gen_chains end

"""
    transition_params_and_stats(model::BUGSModel, sampler, transition)
...
Report only what the sampler actually produced: a statistic that is absent from a draw should
be absent from its `NamedTuple` rather than padded, and statistics may be array-valued. Output
formats that store scalar columns take the union of the keys across draws and fill the gaps
themselves.
