...
examples_to_benchmark = [:rats, :pumps, :oxford, :epil, :lsat, :schools, :beetles, :air]
...
for (i, model_name) in enumerate(examples_to_benchmark)
    ...
    expr = JuliaBUGS._gen_log_density_computation_function_expr(
...
Benchmark._print_results_table(
    juliabugs_source_gen_mooncake_results; backend=Val(:markdown)
)
