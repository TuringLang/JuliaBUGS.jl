...
function benchmark_JuliaBUGS_source_gen(model::JuliaBUGS.BUGSModel, log_density_eval_function::Function; backend_str::Symbol)
    ...
    return BenchmarkResult(:juliabugs_source_gen, LogDensityProblems.dimension(model), density_time, gradient_time)
    elseif backend_str == :Enzyme
        ...
        gradient_time = Chairmarks.@be gradient(f, $prep, $backend, $p, $(Constant(model.evaluation_env)))
        return BenchmarkResult(:juliabugs_source_gen, LogDensityProblems.dimension(model), density_time, gradient_time)
    else
