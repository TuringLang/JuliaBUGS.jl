using Test
using JuliaBUGS
using JuliaBUGS: @bugs, compile, @varname, Gibbs, MHFromPrior, verify_sampler_map, expand_variables
using AbstractMCMC
using Random
using Statistics
...
@testset "Gibbs" begin
    ...
    @testset "Gibbs sampling integration" begin
        ...
        @testset "Basic Gibbs construction" begin
            ...
            @test length(gibbs.sampler_map) == length(model.graph_evaluation_data.sorted_parameters)
    ...
    @testset "Gibbs correctness tests" begin
        @testset "Conjugate Normal-Normal model" begin
            ...
            gibbs = Gibbs(model, MHFromPrior())
            chain = sample(rng, model, gibbs, 50000; chain_type=Chains, discard_initial=10000)
        ...
        @testset "Linear regression convergence" begin
            ...
            rng = Random.MersenneTwister(42)
            gibbs = Gibbs(model, MHFromPrior())
            chain = sample(rng, model, gibbs, 20000; chain_type=Chains, discard_initial=5000)
        ...
        @testset "Compare Gibbs with HMC" begin
            ...
            gibbs = Gibbs(model, MHFromPrior())
            chain_gibbs = sample(rng1, model, gibbs, 5000; chain_type=Chains, discard_initial=1000)
        ...
        @testset "Gibbs with mixed samplers" begin
            ...
            sampler_map = OrderedDict(
                ...
                [@varname(μ), @varname(σ)] => MHFromPrior()
            )
            gibbs = Gibbs(model, sampler_map)
        ...
        @testset "Conditioning correctness" begin
            ...
            @testset "Update only a" begin
                ...
                @test env2.a != initial_params.a || env2.b != initial_params.b || env2.c != initial_params.c
            end
        end
    end
