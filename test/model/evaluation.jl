...
function test_bugs_model_log_density(model_example, expected_untransformed, expected_transformed=expected_untransformed)
    ...
    @test LogDensityProblems.logdensity(transformed_model, JuliaBUGS.getparams(transformed_model)) ≈ expected_transformed rtol = 1E-6
    @test LogDensityProblems.logdensity(untransformed_model, JuliaBUGS.getparams(untransformed_model)) ≈ expected_untransformed rtol = 1E-6
end
...
@testset "evaluate_with_rng!! - controlling sampling behavior" begin
    ...
    eval_env, log_densities = JuliaBUGS.evaluate_with_rng!!(Random.default_rng(), model; sample_all=false)
    @test eval_env.y == 1.0
    @test eval_env.x != 1.0

    eval_env, log_densities = JuliaBUGS.evaluate_with_rng!!(Random.default_rng(), model; sample_all=true)
    @test eval_env.y != 1.0
    @test eval_env.x != 1.0
...
@testset "logprior and loglikelihood" begin
    @testset "Complex model with transformations" begin
        ...
        @test log_densities.tempered_logjoint ≈ log_prior_true + 2.0 * log_likelihood_true rtol = 1E-6
    end
end
...
@testset "Log density" begin
    @testset "Log density of distributions" begin
        @testset "dbin (Binomial)" begin
            ...
            reference_logp_transformed = logpdf(dist, test_θ) + logabsdetjac(Bijectors.inverse(b), test_θ_transformed)
            ...
            @test LogDensityProblems.logdensity(transformed_model, JuliaBUGS.getparams(transformed_model)) ≈ reference_logp_transformed rtol = 1E-6
            @test LogDensityProblems.logdensity(untransformed_model, JuliaBUGS.getparams(untransformed_model)) ≈ reference_logp_untransformed rtol = 1E-6
        end
...
@testset "Log density of BUGS models" begin
    @testset "rats" begin
        test_bugs_model_log_density(JuliaBUGS.BUGSExamples.VOLUME_1.rats, -174029.38703951868)
    end

    @testset "blockers" begin
        test_bugs_model_log_density(JuliaBUGS.BUGSExamples.VOLUME_1.blockers, -8418.416388326123)
    end

    @testset "bones" begin
        test_bugs_model_log_density(JuliaBUGS.BUGSExamples.VOLUME_1.bones, -161.6492002285034)
    end

    @testset "dogs" begin
        test_bugs_model_log_density(JuliaBUGS.BUGSExamples.VOLUME_1.dogs, -1243.188922285352, -1243.3996613167667)
    end
end
...
@testset "evaluate_with_values!!" begin
    @testset "Basic functionality - transformed space" begin
        ...
        @test log_densities.tempered_logjoint ≈ log_densities.logprior + log_densities.loglikelihood
    end
...
@testset "Complex models with multiple parameter types" begin
    @testset "Model with array parameters" begin
        ...
        eval_env, log_densities = JuliaBUGS.evaluate_with_rng!!(rng, model; sample_all=false)
