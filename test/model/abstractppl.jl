using Test
using JuliaBUGS
using JuliaBUGS.Model: condition, decondition, parameters, set_evaluation_mode, UseGeneratedLogDensityFunction, UseGraph
...
@testset "AbstractPPL interface" begin
    @testset "condition" begin
        @testset "Generated function regeneration" begin
            ...
            @test model.log_density_computation_function !== model_cond.log_density_computation_function
        ...
        @testset "Array model conditioning with correct parameter ordering" begin
            ...
            @test parameters(model_cond) == [@varname(x[2]), @varname(y[1]), @varname(y[2]), @varname(y[3])]
        end
        ...
        @testset "Multiple conditioning steps" begin
            ...
            @test model.log_density_computation_function !== model_cond1.log_density_computation_function
            
            # Second conditioning
            model_cond2 = condition(model_cond1, (; b = 1.0))
            @test model_cond1.log_density_computation_function !== model_cond2.log_density_computation_function
            @test model.log_density_computation_function !== model_cond2.log_density_computation_function
            ...
            # Test evaluation
            model_cond2_gen = set_evaluation_mode(model_cond2, UseGeneratedLogDensityFunction())
    ...
    @testset "decondition" begin
        @testset "Basic deconditioning" begin
            ...
            @test model.log_density_computation_function !== model_cond.log_density_computation_function
            @test model_cond.log_density_computation_function !== model_decond.log_density_computation_function
            @test !isnothing(model_decond.log_density_computation_function)
            
            # Test evaluation
            model_decond_gen = set_evaluation_mode(model_decond, UseGeneratedLogDensityFunction())
        ...
        @testset "Full deconditioning to base model" begin
            ...
            @test model_cond2.log_density_computation_function !== model_restored.log_density_computation_function
            ...
            # Test evaluation
            model_restored_gen = set_evaluation_mode(model_restored, UseGeneratedLogDensityFunction())
        ...
        @testset "Error handling" begin
            ...
            @test_throws ArgumentError("Model has no base_model. Use decondition(model, vars) to specify variables to decondition.") decondition(model)
            
            # Cannot decondition originally observed data
            model_cond = condition(model, (; x = 0.5))
            @test_throws ArgumentError("Cannot decondition y: it was observed in the original data") decondition(model_cond, [@varname(y)])
        end
    end
