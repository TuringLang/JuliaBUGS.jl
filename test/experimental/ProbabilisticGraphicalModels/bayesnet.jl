...
@testset "BayesianNetwork" begin
    ...
    @testset "Marginalization for Discrete Variables" begin
        ...
        @testset "evaluate_with_marginalization with parameter values" begin
            ...
            weighted_likelihood = log(
                p_z0 * exp(z0_likelihood_only) + p_z1 * exp(z1_likelihood_only)
            )
            ...
            println(
                "Difference between margin_logp and manual_logp: $(margin_logp - manual_logp)",
            )
            println(
                "Difference between margin_logp and computed_logp: $(margin_logp - computed_logp)",
            )
        end
        ...
        @testset "Mixed discrete and continuous model with dynamic programming" begin
            ...
            # z1=0, z2=0
            z00_bn = set_observations(
                bn, Dict(z_vars[1] => 0, z_vars[2] => 0, y1_var => 1.2, y2_var => 2.3)
            )
            z00_env, z00_logp = evaluate_with_values(z00_bn, params)

            # z1=0, z2=1
            z01_bn = set_observations(
                bn, Dict(z_vars[1] => 0, z_vars[2] => 1, y1_var => 1.2, y2_var => 2.3)
            )
            z01_env, z01_logp = evaluate_with_values(z01_bn, params)

            # z1=1, z2=0
            z10_bn = set_observations(
                bn, Dict(z_vars[1] => 1, z_vars[2] => 0, y1_var => 1.2, y2_var => 2.3)
            )
            z10_env, z10_logp = evaluate_with_values(z10_bn, params)

            # z1=1, z2=1
            z11_bn = set_observations(
                bn, Dict(z_vars[1] => 1, z_vars[2] => 1, y1_var => 1.2, y2_var => 2.3)
            )
            ...
            joint_log_prob = log(
                exp(z00_like) + exp(z01_like) + exp(z10_like) + exp(z11_like)
            )
        ...
        @testset "DP vs Recursive Marginalization" begin
            ...
            @testset "Simple Bernoulli → Normal model" begin
                ...
                println(
                    "  Recursive time: $(recursive_time) seconds, logp: $(logp_recursive)"
                )
        ...
        @testset "DP vs Recursive Marginalization" begin
            ...
            function set_observations(
                bn::BayesianNetwork{V,T,F}, observations
            ) where {V,T,F}
                new_is_observed = copy(bn.is_observed)
                new_evaluation_env = deepcopy(bn.evaluation_env)
            ...
            for n_states in [2, 3, 4]
                @testset "HMM with $n_states states" begin
                    ...
                    println(
                        "$n_states-state HMM: Recursive $(recursive_time)s, DP $(dp_time)s"
                    )
