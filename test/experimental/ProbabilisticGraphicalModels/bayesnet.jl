...
@testset "BayesianNetwork" begin
    ...
    function create_dense_hmm_network(n_states::Int, seq_length::Int)
        ...
        add_stochastic_vertex!(
            bn,
            z_vars[1],
            (_, _) -> Categorical(ones(n_states) / n_states),
            false,
            :discrete,
        )
    ...
    """
    	run_hmm_test_case(n_states, seq_length, expected_memos)

    Runs a single test case for a given HMM configuration. It checks that
    each ordering heuristic produces the correct log-probability and the
    expected memoization cache size.
    """
    function run_hmm_test_case(n_states::Int, seq_length::Int, expected_memos::Dict)
        params = Float64[]

        @testset "Dense HMM: $n_states states, Length $seq_length" begin
            # 1. SETUP: Create the network and get the reference value
            bn = create_dense_hmm_network(n_states, seq_length)
            _, reference_logp = marginalize_without_memo(deepcopy(bn), params)

            # 2. TEST: Loop through each heuristic
            for heuristic in [:dfs, :min_degree, :min_fill]
                @testset "Heuristic: $heuristic" begin
                    sorted_nodes_ids = topological_sort_with_heuristic(bn, heuristic)
                    minimal_keys_for_order = _precompute_minimal_cache_keys(
                        bn, sorted_nodes_ids
                    )
    ...
    @testset "All HMM Configurations" begin

        # Define all the test cases you want to run here.
        # For new cases, use 0 as a placeholder for memo sizes.
        # The failing test will tell you the correct value to put in.
        test_configurations = [
            (
                n_states=3,
                seq_length=14,
                expected_memos=Dict(:dfs => 26224, :min_degree => 226, :min_fill => 226),
            ),
            ...
            (
                n_states=3,
                seq_length=8,
                expected_memos=Dict(:dfs => 952, :min_degree => 118, :min_fill => 118),
            ),
        ]

        # Loop through the configurations and run the test for each one
        for config in test_configurations
            run_hmm_test_case(config.n_states, config.seq_length, config.expected_memos)
        end
    end
end
