...
@testset "BayesianNetwork" begin
    ...
    @testset "Dynamic Programming in Marginalization" begin
        ...
        @testset "Performance with Deep Discrete Graph" begin
            ...
            @info "Marginalization Performance" without_memo=t1 with_memo=t2 speedup=t1/t2 memo_size=memo_size
        ...
        @testset "Correctness with Tricky Dependency Graph" begin
            ...
            add_stochastic_vertex!(original_bn, x_var, (_, _) -> Bernoulli(0.3), false, :discrete)
            add_stochastic_vertex!(original_bn, y_var, (_, _) -> Bernoulli(0.7), false, :discrete)
            loop_vars[x_var] = (;)
            loop_vars[y_var] = (;)
            ...
            @info "Complex graph memo statistics" memo_size=memo_size theoretical_max=4*2^4
    ...
    @testset "Dynamic Programming Performance Analysis" begin
        ...
        @testset "Chain Network Scaling" begin
            ...
            # Test that all speedups are significant 
            @test all(speedups .> 10.0)
            
            # Analyze the relationship between memo size and state count
            for i in 1:length(chain_lengths)
                # The pattern is memo_size = 2^(n+1) - 1 = 2*2^n - 1
                # This is because our memoization approach creates entries based on
                # both the node position and the state of all variables
                expected_memo_size = 2^(chain_lengths[i] + 1) - 1
            ...
            # Interesting pattern: For chains, we observe that speedup decreases as length increases
            # This is because the overhead of memoization grows relative to the computation saved
            # This is a common pattern in DP when we start with extremely high speedups
            if all(diff(speedups) .< 0)
                @info "Note: Chain speedups decrease with length due to increasing memoization overhead relative to computation saved"
            end
        end
