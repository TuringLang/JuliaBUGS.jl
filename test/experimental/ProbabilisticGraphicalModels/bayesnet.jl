...
@testset "BayesianNetwork" begin
    ...
    @testset "Variable Elimination - Three Node Chain" begin
        ...
        add_stochastic_vertex!(bn, :B, Bernoulli(0.8), false)  # temporary distribution
        add_edge!(bn, :A, :B)
        
        # Node C: Depends on B
        # P(C=1|B=1) = 0.9, P(C=1|B=0) = 0.1
        add_stochastic_vertex!(bn, :C, Bernoulli(0.9), false)  # temporary distribution
        add_edge!(bn, :B, :C)
        ...
        # Verify the result
        # We can compute the true marginal P(C=1) by hand:
        # P(C=1) = Σ_A,B P(C=1|B)P(B|A)P(A)
        # = 0.7 * 0.8 * 0.9 + 0.7 * 0.2 * 0.1 + 0.3 * 0.8 * 0.9 + 0.3 * 0.2 * 0.1
