...
function print_pure_text_table(result_dict)
    ...
    println(rpad("Example Name", 25), "|", lpad("Category", 20), "|", lpad("Median Time", 15), "|", lpad("Minimum Time", 15), "|", lpad("Maximum Time", 15), "|", lpad("Memory Usage", 15))
    ...
    for (name, benchmarks) in result_dict
        ...
        for (category, results) in benchmarks
            if first_category
                println(rpad(name, 25), "|", lpad(category, 20), "|", lpad(results["median"], 15), "|", lpad(results["minimum"], 15), "|", lpad(results["maximum"], 15), "|", lpad(results["memory"], 15))
                first_category = false
            else
                println(rpad("", 25), "|", lpad(category, 20), "|", lpad(results["median"], 15), "|", lpad(results["minimum"], 15), "|", lpad(results["maximum"], 15), "|", lpad(results["memory"], 15))
            end
        end
        println("-"^105)  # Adjust the number based on the total length of the header
    end
