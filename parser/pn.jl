...
# Protocol:
# 1. `current_index` is always the token that is being processed
# 2. assume token stream end with `EndMarker`, this requires push `EndMarker` to the end of the token vector
# 2. always call `skip_trivia!` before dispatching to other functions
